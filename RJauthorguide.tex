...
Continuing from \emph{R News}, \emph{The R Journal} will also have a \emph{News and notes} section,
...
\item[Miscellanea:] Short articles (at most two (2) text pages in length)
calling attention to developments or resources of interest to R developers
or users but that are outside the scope of other sections of \emph{The R Journal}.
...
The \pkg{RJournal} \LaTeX{} package provides a much simplified version of the
...
Note that no markup is necessary to typeset R.
...
\item If appropriate, run \code{pdflatex} then
 \code{bibtex} on \file{RJwrapper.tex}
