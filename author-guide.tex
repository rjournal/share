...
\fancyhf{}
\fancyhead[L]{\textsc{Instructions for Authors using LaTeX}}
\fancyhead[R]{\thepage}

\begin{article}
\title{Instructions for Authors using LaTeX}

\author{by The R Journal Editors}

\maketitle
\abstract{
While \emph{The R Journal} strongly prefers submissions using Rmarkdown via the template in the \texttt{rjtools} package, we also allow authors to submit their articles as \LaTeX{} documents. Note that such submissions may be processed more slowly than Rmarkdown submissions. Here we provide such authors with information for preparing \LaTeX{} submissions to the journal.
}
...
and \file{RJournal.sty} from \url{https://github.com/rjournal/share}
...
\item Then submit the following files to the Editor-in-Chief:
  \begin{itemize}
  \item If the submission is of the ``add-on packages'' type, a motivating letter explaining how you meet our requirements.
  ...
  \item You should have only one \file{.bib} file, and only two \file{.tex} files, one of which is \file{RJwrapper.tex}.
  \end{itemize}
\end{itemize}

\emph{Article length}: The \emph{R Journal} has no page limit. However, there is a tendency for longer papers to be subject to longer reviewing time, and excessive length may reduce your article's readership, should it be published. If possible, try to limit your article's length to, say, 8--10 pages, excluding figures and bibliography
...
The BibTeX file should be minimal, that is should not numerous entries not cited in the submission, and the included entries should not contain unnecessary fields.
    ...
    The first time a package is mentioned in the text, excluding the abstract, it should be followed by a formal citation, such as the one generated by the \verb|citation()| function in R. Further mentions of a package do not need to be followed by a citation.

    All packages mentioned in the text should use \verb|\CRANpkg{pkgname}| (for CRAN packages) and \verb|\BIOpkg{pkgname}| (for Bioconductor packages).

    The BibTeX entry for an R package should not use markup of the package name in the \code{title} field, but should
    be produced using \code{toBibtex(citation("pkgname"))}.
...
\item v0.16 2019/08/26 --- Noted importance of strictly following the above rules, and stated there
must be only one \file{.bib} file, and only one \file{.tex} file other than \file{RJwrapper.tex}.
\item v0.17 2019/09/07 --- Added a comment about article length.
\item v0.17 2019/09/11 --- Added a comment about need to provide datasets.

\item v0.18 2025/05/21 --- Removed general information about the journal, and made this document only about \LaTeX\ submissions.
\end{itemize}
